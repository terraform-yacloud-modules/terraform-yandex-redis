...
| <a name="input_notify_keyspace_events"></a> [notify\_keyspace\_events](#input\_notify\_keyspace\_events) | Select the events that Redis will notify among a set of classes. Leave null (default) to not send the parameter to the Yandex Cloud API; sending an empty string is treated by the API as "no change" and fails with `FailedPrecondition: no changes detected` on clusters where the field is unset (e.g. after import). | `string` | `null` | no |
